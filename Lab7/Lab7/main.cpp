...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
	...
	for(int loop = 0; loop < numMoles; loop++)
	{
		...
		molePos = D3DXVECTOR3(spritePositions[(rand() % 14 + 1)], spritePositions[(rand() % 14 + 1)], 0);
	...
	aSurface = d3dMgr->getD3DSurfaceFromFile("Images\\background.jpg");
			...
			*/
			if(timeElapsed > fpsRate)
			{
				for(iter = aMole.begin(); iter != aMole.end(); ++iter)
				{
					...
					aMolePos = (*iter)->getSpritePos();  // get the position of the current balloon
					/*
					==============================================================
